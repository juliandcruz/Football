        ...
        for m in matches_data:
            ...
            if not df_hist.empty and {'HomeTeam', 'AwayTeam', 'FTHG', 'FTAG'}.issubset(df_hist.columns):
                ...
                if len(h_term) <= 2 and len(home.split()) > 1:
                    h_term = home.split()[1].lower()
                if len(a_term) <= 2 and len(away.split()) > 1:
                    a_term = away.split()[1].lower()

                home_games = df_hist[df_hist['HomeTeam'].str.contains(h_term, na=False)]
                away_games_as_home = df_hist[df_hist['AwayTeam'].str.contains(h_term, na=False)]
                
                away_games = df_hist[df_hist['AwayTeam'].str.contains(a_term, na=False)]
                home_games_as_away = df_hist[df_hist['HomeTeam'].str.contains(a_term, na=False)]
            ...
            fair_g = 1 / prob_goals
            odds_g = round(fair_g * (0.92 + rand_factor), 2)
